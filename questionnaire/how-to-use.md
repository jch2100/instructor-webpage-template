# 강사 웹페이지 제작 — 사용 가이드 (대안 워크플로우)

> ⚠️ **이 문서는 대안 워크플로우입니다.**
> Claude Code · Codex · Antigravity 같은 코딩 에이전트를 사용 중이라면 루트의 `README.md`와 `AGENTS.md`를 따르세요. 인터뷰부터 사이트 생성까지 한 번에 끝납니다.
>
> 이 문서는 **에이전트 없이** ChatGPT/Claude.ai 일반 채팅으로 인터뷰만 따로 진행하고, 결과 파일을 별도 시스템(Google Drive 등)으로 옮긴 뒤 수동으로 가져오는 경우에 참고합니다.
